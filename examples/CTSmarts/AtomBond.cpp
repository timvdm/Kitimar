...
bool isCarbonOfDegree3(auto &mol, auto atom)
{
    ...
    return CTSmarts::atom<"[CD3]">(mol, atom);
}

bool isCarboxylCarbon(auto &mol, auto atom)
{
    return CTSmarts::atom<"C(=O)O">(mol, atom); // atom matches first SMARTS atom
}

bool isCarboxylDoubleBond(auto &mol, auto bond)
{
    return CTSmarts::bond<"O=CO">(mol, bond); // bond matches first SMARTS bond
}
...
int main()
{
    ...
    std::cout << "Bond 1 matches \"C=O\": "
              << CTSmarts::bond<"C=O">(molecule, get_bond(molecule, 1)) << std::endl;

    std::cout << "Bond 0 matches \"C=O\": "
              << CTSmarts::bond<"C=O">(molecule, get_bond(molecule, 0)) << std::endl;
